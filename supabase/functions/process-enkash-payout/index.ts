  ...
  try {
    ...
    await supabase
      ...
      .insert({
        payout_id: payout.id,
        status: "processing",
        message: "Payout processing initiated via EnKash",
        metadata: { gateway: "EnKash", environment, transfer_type }
      });
    ...
    if (instant_settlement) {
      payoutData.instant_settlement = true;
    }
    ...
    await supabase
      ...
      .update({
        ...
        gateway_transaction_id: result.transaction_id || result.id || null,
    ...
    console.error("EnKash payout error:", error);
    return new Response(
      JSON.stringify({
        success: false,
