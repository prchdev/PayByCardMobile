...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const baseUrl = isTestMode
      ? "https://test.ccavenue.com/payout/api"
      : "https://secure.ccavenue.com/payout/api";
    ...
    await supabase
      ...
      .insert({
        payout_id: payout.id,
        status: "processing",
        message: "Payout processing initiated via CCAvenue",
        metadata: { gateway: "CCAvenue", environment, transfer_type }
      });

    const payoutData: any = {
      merchant_id: merchantId,
      access_code: accessCode,
      reference_no: payoutReference,
      amount: netAmount.toFixed(2),
      currency: "INR",
      beneficiary_account: beneficiary.account_number,
      beneficiary_ifsc: beneficiary.ifsc_code,
      beneficiary_name: beneficiary.account_holder_name,
      transfer_mode: transfer_type,
      remarks: `Payout for ${payoutReference}`
    };

    if (instant_settlement) {
      payoutData.instant_settlement = "Y";
    }
    ...
    await supabase
      ...
      .update({
        ...
        gateway_transaction_id: result.transaction_id || result.reference_no || null,
    ...
    if (response.ok && (result.status === "SUCCESS" || result.status === "success")) {
      await supabase
        ...
        .update({
          ...
          utr_number: result.utr || result.utr_number || null
        })
        .eq("id", payout.id);
      ...
      await supabase
        ...
        .insert({
          ...
          metadata: { transaction_id: result.transaction_id, utr: result.utr }
        });
      ...
      const failureReason = result.message || result.error || "Payout failed";
    ...
    console.error("CCAvenue payout error:", error);
    return new Response(
      JSON.stringify({
        success: false,
