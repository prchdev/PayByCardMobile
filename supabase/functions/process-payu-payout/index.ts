  ...
  try {
    ...
    await supabase
      ...
      .insert({
        payout_id: payout.id,
        status: "processing",
        message: "Payout processing initiated via PayU",
        metadata: { gateway: "PayU", environment, transfer_type }
      });

    const hashString = `${merchantKey}|${payoutReference}|${netAmount.toFixed(2)}|${beneficiary.account_number}|${beneficiary.ifsc_code}|${salt}`;
    const hash = createHash("sha512").update(hashString).digest("hex");

    const payoutData: any = {
      key: merchantKey,
      txnid: payoutReference,
    ...
    await supabase
      ...
      .update({
        ...
        gateway_transaction_id: result.payuMoneyId || result.txnid || null,
        gateway_status_code: result.status || null
      })
      .eq("id", payout.id);

    if (response.ok && (result.status === "success" || result.status === 1)) {
      await supabase
        .from("payouts")
        .update({
          status: "completed",
          completed_at: new Date().toISOString(),
          utr_number: result.utr || result.bankRRN || null
        })
        .eq("id", payout.id);
      ...
      await supabase
        ...
        .insert({
          ...
          metadata: { transaction_id: result.payuMoneyId, utr: result.utr }
        });
      ...
      const failureReason = result.msg || result.error_Message || "Payout failed";
    ...
    console.error("PayU payout error:", error);
    return new Response(
      JSON.stringify({
        success: false,
